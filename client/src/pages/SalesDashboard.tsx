...
//       const url = `${import.meta.env.VITE_BASE_URL}/sales/getAddedUsers?salesId=${currentUserId}`;
...
//       // Determine Role Type for Backend
//       let backendRoleType = "BUILDER";
//       if (formData.role === "Broker") {
//         backendRoleType = "BROKER";
//       } else if (formData.role === "Charted Accountant") {
//         backendRoleType = "CA";
//       }
...
//       // Only add Company Name and RERA ID if role is Builder
//       if (backendRoleType === "BUILDER") {
//         roleDataPayload.companyName = formData.companyName.trim();
//         roleDataPayload.reraId = formData.reraId;
//       }
...
//       // UPDATED: Dynamic Toast Notification
//       toast({
//         title: "Success",
...
//               Manage your members and track performance
//             </p>
//           </div>
//           <Button
...
//               <p className="text-black text-lg">No member added yet</p>
...
const SalesDashboard = () => {
  ...
  // Renamed builders to members to support all roles
  const [members, setMembers] = useState([]); 
  const [filterRole, setFilterRole] = useState("BUILDER"); // Default filter
  ...
  }, [user?.userId, filterRole]); // Refetch when role filter changes
  ...
  // Updated function to fetch members based on role
  const fetchMembers = async () => {
    try {
      ...
      // New API Endpoint
      const url = `${import.meta.env.VITE_BASE_URL}/sales/getAddedUsers?salesId=${currentUserId}&role=${filterRole}`;
  ...
  const fetchMemberLeads = async (memberId) => {
    try {
      setLoadingLeads(true);
      setLeadsError(null);
      setMemberLeads([]);
      ...
      const url = `${import.meta.env.VITE_BASE_URL}/leads/get/${memberId}`;
      const response = await fetch(url, {
        method: "GET",
        headers: {
          Authorization: `Bearer ${token}`,
      ...
      const data = await response.json();
      setMemberLeads(Array.isArray(data) ? data : []);
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // If the newly added member matches current filter, refetch
      if(backendRoleType === filterRole) {
          fetchMembers();
      }
      ...
      <div className="p-6">
        ...
        <GlassCard className="p-4 bg-white border-gray-200">
          ...
          {loading ? (
            <div className="text-center py-8">
              <div className="w-16 h-16 border-4 border-gray-400 border-t-black rounded-full animate-spin mx-auto mb-2"></div>
              <p className="text-black text-lg">Loading members...</p>
            </div>
          ) : error ? (
            <div className="text-center py-8">
              <div className="bg-red-500/10 border border-red-500/30 rounded-lg p-4 mb-4">
            ...
            <div className="text-center py-8">
              <Building2 className="w-16 h-16 text-gray-400 mx-auto mb-2" />
              <p className="text-black text-lg">No {filterRole.toLowerCase().replace('_', ' ')}s added yet</p>
              <p className="text-gray-500 text-sm mt-1">
                Click "Add Member" to get started
              </p>
            </div>
            ...
            <div className="flex flex-col divide-y divide-black">
              {members.map((member, index) => (
                <div
                  ...
                  className={`py-3 px-4 hover:bg-gray-100 transition-all flex flex-col gap-1 cursor-pointer ${selectedMember?.userId === (member.userId || member.id) ? "bg-gray-100" : ""}`}
                  onClick={() => {
                    setSelectedMember(member);
                    fetchMemberLeads(member.userId || member.id); // Assuming ID field is userId or id
                  }}
                >
                  <div className="flex items-center gap-2 mb-1">
                    <h3 className="text-lg font-semibold text-black/90">
                  ...
                  {member.reraId && (
                    <p className="text-black text-xs mt-1">
                      RERA ID:{" "}
                      <span className="text-black">{member.reraId}</span>
                    </p>
                  )}
                </div>
              ))}
        ...
        {selectedMember && (
          <GlassCard className="mt-2 p-4 bg-white border-gray-200">
            <h2 className="text-xl font-bold text-black mb-4">
              Leads for {selectedMember.fullName || `${selectedMember.firstName} ${selectedMember.lastName}`}
            </h2>
            ...
            {loadingLeads ? (
              ...
              <p className="text-black">No leads registered for this member</p>
            ) : (
              <div className="overflow-x-auto">
                <table className="min-w-full text-sm text-left border border-gray-700 rounded-lg overflow-hidden">
                  <thead className="bg-white text-black">
